<script>
  ...
  import {deleteDailyDataItem, currDataItemFromStoredData, featureCollectionFromStoredData} from '$lib/editor.svelte.js';
  ...
  $effect(()=> {
    $dailyData = $currDate && featureCollectionFromStoredData($storedData);
  })
  ...
  const deleteItem = (id)=> {
    console.log(id)
    deleteDailyDataItem($dailyData, id);
  };
...
<ModalNew
  ...
  modalClasses = "h-full w-full p-0 bg-yellow-100 items-start"
>
  <div class="flex items-center justify-start gap-4 p-2 text-lg font-bold sm:text-xl bg-lime-300">
    Daily List
  ...
  <div class="w-full h-full divide-y bg-lime-100 divide-slate-400">
    <ul
      ...
      {#each dataList.features as item, i (item.properties.id)}
        ...
        <li class="text-lg select-none bg-violet-100" use:scroll>
          ...
          <div tabindex="0" class="min-h-0 p-0 px-1 collapse">
            <div class="grid grid-cols-[1fr_auto] h-auto min-h-0 gap-2 px-2 py-1 collapse-title">
              <div>
              ...
              <div>{item.properties.disp.no}</div>
